...
from visualizations import analyze_visualization
...
def sort_button_clicked():
    ...
    elif selected_algorithm == "All Algorithm":
        #sorting_algorithm = 
        return
...
def view_visualization():
    input_str = input_entry.get()
    input_array = list(map(int, input_str.split()))
    analyze_visualization(input_array)

# Create the main wxindow
root = tk.Tk()
...
#Create a button for analyze
analyze_button = tk.Button(root, text="Analyze Efficiency", command=view_visualization)
analyze_button.pack()
